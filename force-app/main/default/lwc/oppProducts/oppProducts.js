...
export default class OppProducts extends LightningElement {
  @api recordId;
  ...
  getUserInfo() {
    ...
    query({
      q:
        "SELECT Id, ProfileId, Profile.Name FROM User WHERe Id ='" +
        userId +
        "'"
    }).then((result) => {
  ...
  getRecord(result) {
    ...
    if (result.data) {
      ...
      if (this.opportunity.data.recordTypeInfo.name == "Spare Parts") {
  ...
  queryLineItems() {
    ...
    query({
      q:
        "SELECT Id, Name, New_Product__c ,Total_Price_Discount__c, Description2__c, Quantity_Remaining__c,Cost_Price__c, List_Price__c, UOM__c, Opportunity.Warehouse__r.code__c, Product2Id ,Product2.Name, Product2.ProductCode, Product2.Description, Product2.UOM__c, Product2.M_I_Configuration__c, Product2.Cost_Price__c, Amount_In_Supplier_Currency__c, Quantity, Selling_Factor__c, Margin__c, UnitPrice, ListPrice, TotalPrice, Discount FROM OpportunityLineItem WHERE OpportunityId = '" +
        this.recordId +
  ...
  querySellingFactors() {
    ...
    if (this.opportunity && this.opportunity.data) {
      query({
        q:
          "SELECT Dealer_Selling_Factor__c,End_User_Selling_Factor__c,Price_Range_From__c,Price_Range_To__c FROM Selling_Factor__c WHERE Active__c = TRUE AND Supplier__c = '" +
          this.opportunity.data.fields.Supplier__c.value +
          "' AND Ship_To__c = '" +
          this.opportunity.data.fields.Ship_To__c.value +
  ...
  queryRecentlyViewedProducts() {
    ...
    search({
      q:
        "SELECT Id, Name, ProductCode, UOM__c FROM Product2 ORDER BY LastViewedDate DESC NULLS LAST LIMIT 5",
      sObjectType: "Product2",
  ...
  get supplierCurrency() {
    return this.opportunity &&
      ...
      this.opportunity.data.fields.Supplier__r.value
      ? this.opportunity.data.fields.Supplier__r.value.fields.Currency__c
        .displayValue
      : null;
  }
  ...
  get lineItems() {
    ...
    if (
      this.opportunity &&
      this.opportunity.data &&
      this.opportunity.data.recordTypeInfo.name ==
      "Customised Engineering Solution" &&
      this.items &&
      this.add7Default == true
    ) {
      //&& this.items.length == 0)
  ...
  get isCES() {
    return this.opportunity &&
      this.opportunity.data &&
      this.opportunity.data.recordTypeInfo.name != "Spare Parts"
      ? true
      : false;
  }
  ...
  inputChanged(event) {
    ...
    this.items = clone.map((item) => {
      if (item.index == event.target.dataset.index) {
        ...
        if (this.opportunity.data.recordTypeInfo.name == "Spare Parts") {
          item.Amount_In_Supplier_Currency__c =
            item.Amount_In_Supplier_Currency__c || 0;
          item.Selling_Factor__c = item.Selling_Factor__c || 0;
          if (event.target.name == "Amount_In_Supplier_Currency__c") {
            let sellingFactor = this.sellingFactors.find(
              (i) =>
            ...
            if (sellingFactor) {
              let customCategory = this.opportunity.data.fields.Account.value
                .fields.Customer_Category__c.value;
              if (customCategory == "Dealer") {
                item.Selling_Factor__c = sellingFactor.Dealer_Selling_Factor__c;
              } else if (customCategory == "End User") {
                item.Selling_Factor__c =
                  sellingFactor.End_User_Selling_Factor__c;
              }
            }

            item.UnitPrice =
              item.Amount_In_Supplier_Currency__c * item.Selling_Factor__c;
            item.UnitPrice = parseFloat(item.UnitPrice).toFixed(2);
          }
        } else if (this.opportunity.data.recordTypeInfo.name != "Spare Parts") {
  ...
  handleSelectionChange(event) {
    ...
    let Customer_Category = this.opportunity.data.fields.Customer_Category__c.value;
    let createdDate = this.opportunity.data.fields.CreatedDate.value;
  ...
  saveProducts(event) {
    ...
    console.log("validPicklist : " + validPicklist);
    if (allValid && validPicklist) {
      let sobjs = this.items.map((item) => {
        ...
        if (item.Product2Id && typeof item.Product2Id != "undefined") {
          ...
          if (item.isNew) {
            ...
            sobj.OpportunityId = item.OpportunityId;
          } else {
            sobj.Id = item.Id;
          }
      ...
      this.newItems.forEach(item => {
        ...
        if (item.Product2.Name && typeof item.Product2.Name != "undefined" &&
          ...
          oppProduct.Discount = item.Discount;
          ...
          oppProduct.sobjectType = "OpportunityLineItem";
          oppProduct.OpportunityId = item.OpportunityId;
      ...
      saveAndDelete({ lineItems: sobjs, deleteLineItems: this.removedItems, orderId: null, newProducts: newProducts })
        .then((result) => {
          if (close == 'true') {
            const closeQA = new CustomEvent('close');
            this.dispatchEvent(closeQA);
