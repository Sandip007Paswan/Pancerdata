    ...
    doInit : function(component, event, helper) {
        ...
        action.setCallback(this,function(response){
            ...
            if(state === "SUCCESS")
            {
                ...
                if (record.Oracle_Submission_Status__c == 'Successful')
                    helper.showToast("error","OracleStatus","The Order is already submitted to Oracle");
                else if (record.Oracle_Submission_Status__c == 'In Progress')
                    helper.showToast("error","OracleStatus","In Progress to create the Order in Oracle");
                else if (record.Status != 'Submit')
                    helper.showToast("error","Status","Only Submit Stage can submit to Oracle");
                    ...
                    else if (record.Address__c == null || record.Address__r.ERP_Id__c == null)
                    ...
                    else if (record.Warranty_Start_Date__c == null)
                    ...
                    { 
                        ...
                        {  
                            var itemNo = i+1;
                            if(oppLineItems[i].Product2.New_Product__c == true)
                            {
                                helper.showToast("error","New Product","Cannot submit New Product to Oracle, Product Code "+oppLineItems[i].Product2.ProductCode);
                        ...
                        if(productValidated == true)
                        {
                            ...
                            else if(state === "ERROR")
                            {
                                ...
                                component.set("v.SentSuccess",false);
        ...
        /*-----------------------------------*/
        if(component.get("v.isValidated") === true)
        {
            var action = component.get("c.doProcess");
            action.setParams({"recordId":recordId});
            action.setCallback(this,function(response){
                var state = response.getState();
                if(state === "SUCCESS")
                {
                    var oracleStatus = response.getReturnValue();
                    component.set("v.Status", oracleStatus.status);
                    component.set("v.StatusCode", oracleStatus.statusCode);
                    component.set("v.RequestId", oracleStatus.requestId);
